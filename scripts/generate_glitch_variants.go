...
func main() {
	...
	fmt.Println("=== Generating Pixel Gopher Variants ===")
	...
	for rarityIdx, rarity := range rarities {
		for variant := 1; variant <= 2; variant++ {
			...
			// Save
			filename := fmt.Sprintf("pixel_%s_%d.png", strings.ToLower(rarity), variant)
	...
	fmt.Printf("\n=== Complete ===\n")
	fmt.Printf("Generated 10 pixel gopher variants in: %s\n", outputDir)
}
